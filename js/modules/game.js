...
export async function initGame(hostStatus = false, playerId = 'local_player', lobbyId = null, playersData = null) {
    ...
    // Uložení parametrù do globálního stavu (DÙLEŽITÉ!)
    ...
    // Vyèištìní logu pøes UI modul
    const logEl = document.getElementById('log-container');
    if (logEl) logEl.innerHTML = '';

    removeContextMenu();

    // INICIALIZACE HRÁÈÙ Z LOBBY DATA (ROZDÁVÁNÍ KARET)
    gameState.players = {};

    if (playersData) {
        // Seøadit hráèe konzistentnì (napøíklad abecednì podle klíèe = poøadí pøipojení do Firebase)
        const playerIds = Object.keys(playersData).sort();

        playerIds.forEach((id, index) => {
            if (index >= C.MAX_PLAYERS) return; // Záchrana proti pøeplnìní mapy
            ...
            gameState.players[id] = {
                ...
                index: index, // Poøadí slouží pro výpoèet rohù základny
    ...
    console.log("[GAME] Hráèi inicializováni (v162):", gameState.players);
    ...
    // Vracíme Promise, který se vyøeší, až se dokonèí finishInit
    return new Promise((resolve, reject) => {
        // Vytvoøení herního pole - SYNCHRONIZOVANÉ
...
async function syncWorldGeneration(resolve) {
    ...
    if (gameState.isHost) {
        console.log("[WORLD] Hostitel generuje svìt...");
        gameState.gameBoard = [];
        let terrainString = ''; // Base64 / String pro bezpeèný pøenos
        ...
        // Uložit do Firebase pøes bezpeèný JSON string
        console.log("[WORLD] Nahrávám data svìta a budov na server (bezpeèná stringifikace)...");
        ...
        console.log("[WORLD] Data svìta nahrána.");
        finishInit(resolve);
    } else {
        console.log(`[WORLD] Klient (${gameState.myPlayerId}) èeká na data svìta pro session ${gameState.sessionToken}...`);
        ...
        unsubWORLD = onValue(worldRef, (snapshot) => {
            ...
            if (snapshot.exists() && data && data.terrainStr && data.structuresJSON) {
                // Skuteènì checkneme token, abychom nenatáhli starou mapu z minulé relace!
                if (data.sessionToken === gameState.sessionToken || gameState.sessionToken === 'legacy') {
                    console.log("[WORLD] Aktuální Data svìta dorazila! (v167)");
                    ...
                    finishInit(resolve); // Svìt a budovy už máme stažené z Firebase, spouštíme klienta!
                } else {
                    console.log("[WORLD] Ignoruji starý svìt z minulé session...");
                }
            }
        });
    }
...
function generateStructures() {
    // Definovat Base pozice podle indexù hráèù (z konfigurace)
    ...
    for (let i = 0; i < C.NUM_STRUCTURES; i++) {
        ...
        if (rand < 0.35) placeRandomStructure('mine', 2, { name: 'Dùl', income: 5, cost: 100 });
        else if (rand < 0.70) placeRandomStructure('village', 3, { name: 'Vesnice', unit_bonus: 7, cost: 75 });
        else if (rand < 0.85) placeRandomStructure('crystal_mine', 2, { name: 'Krystalový dùl', income: 1, cost: 300 });
        else if (rand < 0.95) placeRandomStructure('ancient_library', 4, { name: 'Prastará knihovna', reveal_radius: 15, cost: 250 });
        else placeRandomStructure('trading_post', 3, { name: 'Tržištì', cost: 150 });
    }
}

function finishInit(resolveCallback) {
    // MULTIPLAYER SYNC: Pøipojit se k odbìru cizích expedic
    if (gameState.currentLobbyId) {
        setupMultiplayerSync();
    }

    // ÚVODNÍ ODHALENÍ MAPY (aby nebyla èerná obrazovka!)
    // Objevíme mapy všech aktivních hráèù okolo jejich základen, ale jen do jejich pohledu
    Object.values(gameState.players).forEach((p) => {
        const basePos = C.BASE_POSITIONS[p.index];
        if (basePos) {
    ...
    // Viewport: Zacílit kameru hráèe na JEHO základnu
    ...
    // Inicializujeme pøíjem, aby hráèi zaèali správnì
    ...
    logMessage(`Vítej v Pixelové Øíši! Verze 165 aktivní. Hraješ jako ${gameState.players[gameState.myPlayerId]?.name || gameState.myPlayerId}.`, 'win');

    gameState.needsRedraw = true;
    requestAnimationFrame(gameLoop);
    requestAnimationFrame(physicsLoop);

    // NYNÍ JE HRA KOMPLETNÌ PØIPRAVENÁ A MÙŽEME ODKRÝT UI
    window.showScreen('game-ui');

    // Zapojení vstupních listenerù (mouse/keyboard events)
    import('../main.js?v=169').then(m => {
        if (window.attachEventListeners) window.attachEventListeners(); // v main.js attach fn wrapper
    });

    // Pojistka pøekreslení plátna pøesnì poté, co dom odryl CSS vrstvu DIVu
    setTimeout(() => {
        const vp = document.getElementById('game-viewport');
...
export function physicsLoop(timestamp) {
    const dt = (timestamp - lastPhysicsTime) / 1000; // vteøiny ubìhlé od minulého framu
    ...
    for (const playerId in gameState.players) {
        ...
        for (let i = player.activeExpeditions.length - 1; i >= 0; i--) {
            ...
            if (!exp.arrived) {
                ...
                // Konstantní rychlost pøepoètená posunutá o Delta Time snímkù vykreslovacích monitorù
            ...
            if (!exp.arrived) {
                ...
                // FOG OF WAR: Lokálnì odhaluje mapu pouze moje vlastní expedice!
    ...
    // Asynchronnì toèíme dokola jak blesk (cca 60-144x za sekundu v závislosti na monitoru)
    requestAnimationFrame(physicsLoop);
}
        ...
        p1.activeExpeditions.forEach(e1 => {
            p2.activeExpeditions.forEach(e2 => {
                ...
                if (dist < 1.5) { // Dosah boje
                    ...
                    // Výpoèet ztrát (zjednodušený Meat Grinder)
...
function redirectExpeditionToHome(playerId, exp) {
    ...
    if (base) {
        if (!exp.isReturning) {
            logMessage(`Expedice #${exp.id} panikaøí a ustupuje k základnì!`, 'warn');
...
function removeExpedition(playerId, expId) {
    ...
    if (player) {
        ...
        logMessage(`Expedice #${expId} byla znièena v boji.`, 'error');
    }
}
...
function handleExpeditionArrival(playerId, exp) {
    ...
    // Dynamický rádius odhalení a záboru v cíli (výraznìjší pro vìtší armády)
    ...
    if (struct) {
        if (struct.ownerId === playerId) {
            ...
            logMessage(`Expedice #${exp.id} dorazila k vlastní budovì ${struct.data.name}.`, 'info');
        } else {
            // Boj o budovu
            const defenderId = struct.ownerId;
            if (defenderId) {
                // TODO: Skuteèný souboj, zatím automatické obsazení
                ...
                logMessage(`Expedice #${exp.id} obsadila opuštìný ${struct.data.name}.`, 'win');
                struct.ownerId = playerId;
                struct.type = 'owned_' + struct.type.replace('visible_', '').replace('hidden_', '');
            }
        }
...
export function launchExpedition(playerId, targetX, targetY, units, sourceX = null, sourceY = null) {
    ...
    // Dynamické urèení startu - buï z parametru, nebo z hlavní základny hráèe
    let finalSourceX = sourceX;
    let finalSourceY = sourceY;

    if (finalSourceX === null || finalSourceY === null) {
        // Hledáme základnu podle ID hráèe
        const base = Array.from(gameState.structures.values()).find(s => s.ownerId === playerId && s.type.includes('base'));
        if (base) {
            finalSourceX = base.x + Math.floor(base.w / 2);
            finalSourceY = base.y + Math.floor(base.h / 2);
        } else {
            console.error(`[CRITICAL] Hráè ${playerId} nemá na mapì žádnou základnu! Jednotky nevyslány.`);
            logMessage("Nemùžeš vyslat jednotky, tvoje základna byla znièena nebo chybí!", 'error');
            return;
        }
    }
...
export function setupMultiplayerSync() {
    ...
    // Tady musíme poslouchat VŠECHNY hráèe aktivní ve høe (kromì lokálního)
    Object.keys(gameState.players).forEach(otherPlayerId => {
        ...
        onValue(expeditionsRef, (snapshot) => {
            ...
            if (otherPlayer) {
                ...
                // Zachováme stávající jednotky (a jejich fyzický progress pohybu) a pøidáme nové
                ...
                for (const id in data) {
                    ...
                    if (existingExp) {
                        ...
                        // Nová expedice od nepøítele, která se právì zrodila
...
export function redirectExpedition(playerId, expId, targetX, targetY) {
    ...
    logMessage(`Expedice #${exp.id} pøesmìrována na [${targetX}, ${targetY}].`);

    // MULTIPLAYER SYNC PØESMÌROVÁNÍ
    if (gameState.currentLobbyId && playerId === gameState.myPlayerId) {
        import('../main.js?v=169').then(m => {
            m.syncExpeditionToFirebase(playerId, exp);
        });
...
export function splitExpedition(playerId, expId, targetX, targetY, percent) {
    ...
    // Vytvoøení nové expedice z odštìpených jednotek
    const newExp = {
        id: ++player.expeditionCounter,
        startX: curX,
        startY: curY,
    ...
    logMessage(`Expedice #${exp.id} rozdìlena! Nová expedice #${newExp.id} vyslána s ${splitUnits} jednotkami.`);

    // MULTIPLAYER SYNC ROZDÌLENÍ A ZMENŠENÍ PÙVODNÍ
    if (gameState.currentLobbyId && playerId === gameState.myPlayerId) {
        import('../main.js?v=169').then(m => {
            m.syncExpeditionToFirebase(playerId, exp);
...
export function gatherExpeditions(playerId, targetX, targetY) {
    ...
    // Tato funkce novì pouze nasmìruje jednotky k sobì. 
    // Fyzické slouèení probìhne plynule v gameTick (Merge Pass), až se k sobì armády pøiblíží.
