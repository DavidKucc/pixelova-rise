console.log('[DEBUG] game.js loaded v=220');
...
import * as C from './config.js?v=220';
import { gameState, viewportState } from './state.js?v=220';
import { ui, updateUI, updateExpeditionsPanel, updateActionPanel, logMessage, createContextMenu, removeContextMenu } from './ui.js?v=220';
import { getServerTime, getNeighbors, isAreaClear, createStructure, placeRandomStructure, findPath } from './utils.js?v=220';
import { gameLoop, initRendererCache, updateFogCache } from './renderer.js?v=220';
import { runAIDecision } from './ai.js?v=220';
import { Logger } from './logger.js?v=220';

// --- MULTIPLAYER (V201 ODDELENO) ---
import { setupMultiplayerSync, syncExpeditionToFirebase, removeExpeditionFromFirebase, syncActionToFirebase } from './multiplayer.js?v=220';
import { handleCombatBetweenExpeditions } from './combat.js?v=220';
...
function finishInit(resolveCallback) {
    ...
    logMessage(`Vítej v Pixelové říši! Verze 220 aktivní. Hraješ jako ${gameState.players[gameState.myPlayerId]?.name || gameState.myPlayerId}.`, 'win');
...
function gameTick() {
    for (const playerId in gameState.players) {
        ...
        if (player.activeExpeditions) {
            ...
            handleMergingBetweenExpeditions(playerId); // v188: Merge Pass
        }
    }
    updateUI();
    updateExpeditionsPanel();
...
function handleMergingBetweenExpeditions(playerId) {
    ...
    // Merge probíhá pouze pokud obě expedice směřují na stejné místo a jsou u sebe
    for (let i = 0; i < player.activeExpeditions.length; i++) {
        for (let j = i + 1; j < player.activeExpeditions.length; j++) {
            const e1 = player.activeExpeditions[i];
            const e2 = player.activeExpeditions[j];

            if (!e1 || !e2) continue;
            if (e1.unitsLeft <= 0 || e2.unitsLeft <= 0) continue; // Neviditelní duchové z předchozích sessionů se nesmí slučovat

            // Musí mít stejný cíl
            if (e1.targetX !== e2.targetX || e1.targetY !== e2.targetY) continue;
            ...
            if (dist < 1.0) {
                // SLOUČENÍ!
                // Očištění starých dat kvůli Firebase JSON restrikcím (NaN by schodilo celý serverový update)
                const safeInitialE1 = isNaN(e1.initialUnits) || e1.initialUnits === undefined ? e1.unitsLeft : e1.initialUnits;
                const safeInitialE2 = isNaN(e2.initialUnits) || e2.initialUnits === undefined ? e2.unitsLeft : e2.initialUnits;

                // Ponecháme e1, přidáme k ní jednotky z e2
                e1.unitsLeft += e2.unitsLeft;
                e1.initialUnits = safeInitialE1 + safeInitialE2;

                logMessage(`Expedice #${e2.id} se sloučila s #${e1.id}. Celkem ${Math.round(e1.unitsLeft)} jednotek.`, 'info');

                // Odstranit e2
                const e2Id = e2.id;
                removeExpedition(playerId, e2Id);

                // Host zapisuje výsledek slučování všech hráčů pro všechny
                if (gameState.currentLobbyId) {
                ...
                gameState.needsRedraw = true;
                return; // Pro jistotu vyskočíme, abychom nenarušili indexy pole uprostřed smyčky
            }
        }
    }
}
...
export function launchExpedition(playerId, targetX, targetY, units, sourceX = null, sourceY = null) {
    ...
    const exp = {
        id: 'exp_' + Date.now() + '_' + Math.floor(Math.random() * 10000), // Bezpečné unikátní ID (řeší kolize po F5)
        startX: finalSourceX,
        startY: finalSourceY,
        targetX,
        targetY,
        ...
        initialUnits: units,
        progress: 0,
        startTime: getServerTime(), // v190 Sync
        duration: duration,
        isHolding: false,
        arrived: false
    };
...
export function redirectExpedition(playerId, expId, targetX, targetY) {
    ...
    exp.startTime = getServerTime(); // v190 Sync
...
export function splitExpedition(playerId, expId, targetX, targetY, percent) {
    ...
    const newExp = {
        id: 'exp_' + Date.now() + '_' + Math.floor(Math.random() * 10000), // Fix F5 kolizí u odštěpených
        startX: curX,
        startY: curY,
        targetX,
        targetY,
        ...
        progress: 0,
        startTime: getServerTime(), // v191 Sync
        duration: duration,
        isHolding: false
    };
        ...
        for (let x = Math.max(0, cx - radius); x <= Math.min(C.GRID_SIZE - 1, cx + radius); x++) {
            ...
            if (dist <= radius) {
                ...
                if (!cell.visibleTo.includes(playerId)) {
                    ...
                    // V218: Dynamický odpar mlhy v Cache plátně pro lokálního hráče
                    if (playerId === gameState.myPlayerId) {
                        updateFogCache(x, y);
                    }
                }
    ...
    for (let y = 0; y < C.GRID_SIZE; y++) {
        for (let x = 0; x < C.GRID_SIZE; x++) {
            if (!gameState.gameBoard[y][x].visibleTo.includes(playerId)) {
                ...
                if (playerId === gameState.myPlayerId) updateFogCache(x, y);
            }
        }
    }
    gameState.needsRedraw = true;
