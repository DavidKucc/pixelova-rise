// js/modules/utils.js
// Pomocné, znovupoužitelné funkce, které nejsou pøímo vázané na herní logiku.
...
export function createStructure(type, x, y, w, h, data, ownerId, externalId = null) {
    ...
    for (let i = y; i < y + h; i++) {
        for (let j = x; j < x + w; j++) {
            ...
            if (cell) {
                // Toto je klíèové: každá buòka, kterou struktura zabírá,
                // musí dostat ID této struktury.
                cell.structureId = id;

                // Pokud je struktura vytváøena s vlastníkem (napø. startovní základna),
                // rovnou mu pøiøadíme i buòky pod ní.
                if (ownerId) {
